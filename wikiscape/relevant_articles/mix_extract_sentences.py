...
split_task_steps = [["add","onion"],["add","rice"],["add","ham"],["add","kimchi"],["pour","sesame","oil"],["stir","mixture"]]
elmo = ElmoEmbedder()    

def load_task_steps(tasks_primary_path):
    with open(tasks_primary_path, "rb") as f:
        # lines 0, 6, 12, 18, ... have the task id
        # lines 4, 10, 16, 22, ... have the task_steps
        all_lines = f.readlines()
        for i in range(NUM_TASKS):
            task_id = all_lines[i*6]
            steps = all_lines[i*6+4].split(",")
            split_steps = [step.split(" ") for step in steps]
            pdb.set_trace()
...
def print_list(sentence_list):
    ...
    for sentence in sentence_list:
        ...
        for (task_step, split_task_step) in zip(task_steps, split_task_steps):
    ...
    best_dp_order = []
    present_task_steps = [ts for ts in task_steps if ts in steps_present]
    ...
    print("Best order: ")
    for step in best_dp_order:
        sentence = ""
        for word in step[2]:
            sentence += word + " "
        print(step[0] + ": " + sentence) 
